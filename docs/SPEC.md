...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'Qorin';
...
Messages must start with the trigger pattern (default: `@Qorin`):
- `@Qorin what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @Qorin` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Qorin what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Qorin what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Qorin add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Qorin remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Qorin list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Qorin remember I prefer dark mode` | Add to global memory |
...
```
User: @Qorin remind me every Monday at 9am to review the weekly metrics
...
```
User: @Qorin at 5pm today, send me a summary of today's emails
...
From any group:
- `@Qorin list my scheduled tasks` - View tasks for this group
- `@Qorin pause task [id]` - Pause a task
- `@Qorin resume task [id]` - Resume a paused task
- `@Qorin cancel task [id]` - Delete a task

From main channel:
- `@Qorin list all tasks` - View tasks from all groups
- `@Qorin schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@Qorin add group "Name"` in main |
